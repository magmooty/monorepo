...
pub async fn initialize_telegram() {
    ...
    let authorization_handler = ConsoleAuthorizationHandler::new(client.clone());
            ...
            match authorization_state.state {
                ...
                AuthorizationState::AuthorizationStateWaitOtherDeviceConfirmation => {
                    authorization_handler
                        .handle_wait_other_device_confirmation(authorization_state.link.unwrap())
                        .await;
                }
                AuthorizationState::AuthorizationStateWaitPassword => {
                    authorization_handler.handle_wait_password(authorization_state.password_hint).await;
                }
