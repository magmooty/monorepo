pub static LOCAL_SCHEMA: &str = "
			DEFINE TABLE user SCHEMAFULL
				PERMISSIONS
					...
					FOR UPDATE WHERE id = $auth.id OR (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center'), // User can update themselves
					FOR CREATE, DELETE WHERE (SELECT * FROM scope WHERE user = $auth.id AND (scope_name = 'manage_center' OR scope_name = 'manage_space')); // Center manager can create other users, A space owner can create users
			...
			DEFINE TABLE scope SCHEMAFULL
				PERMISSIONS
					...
					FOR CREATE, UPDATE, DELETE WHERE (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center') OR (SELECT id FROM scope WHERE user = $auth.id AND scope_name = 'manage_space' AND space = $this.space); // Center manager can modify other users' scopes, A space owner can modify users' scopes if the scopes are for an owned space
        ...
        SIGNIN ( SELECT * FROM user WHERE phone_number = $phone_number AND crypto::argon2::compare(password, $password) );
			...
			DEFINE TABLE space SCHEMAFULL
				PERMISSIONS
					FOR SELECT FULL,
					FOR CREATE, DELETE WHERE (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center'),
					FOR UPDATE WHERE (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center') OR id IN (SELECT space FROM scope WHERE user = $auth.id AND scope_name = 'manage_space');
			DEFINE FIELD name ON TABLE space TYPE string;

			DEFINE TABLE academic_year SCHEMAFULL
				PERMISSIONS
					FOR SELECT WHERE (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center') OR space IN (SELECT space FROM scope WHERE user = $auth.id GROUP BY space).space,
					FOR CREATE, UPDATE, DELETE WHERE (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center') OR space IN (SELECT space FROM scope WHERE user = $auth.id AND scope_name = 'manage_space' GROUP BY space).space OR space IN (SELECT space FROM scope WHERE user = $auth.id AND scope_name = 'manage_academic_years' GROUP BY space);
			...
			DEFINE TABLE academic_year_course SCHEMAFULL
				PERMISSIONS
					FOR SELECT WHERE (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center') OR space IN (SELECT space FROM scope WHERE user = $auth.id GROUP BY space).space,
					FOR CREATE, UPDATE, DELETE WHERE (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center') OR space IN (SELECT space FROM scope WHERE user = $auth.id AND scope_name = 'manage_space' GROUP BY space).space OR space IN (SELECT space FROM scope WHERE user = $auth.id AND scope_name = 'manage_academic_year_courses' GROUP BY space);
			...
			DEFINE TABLE student SCHEMAFULL
				PERMISSIONS
					FOR SELECT WHERE (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center') OR space IN (SELECT space FROM scope WHERE user = $auth.id GROUP BY space).space,
					FOR CREATE, UPDATE, DELETE WHERE (SELECT * FROM scope WHERE user = $auth.id AND scope_name = 'manage_center') OR space IN (SELECT space FROM scope WHERE user = $auth.id AND scope_name = 'manage_space' GROUP BY space).space OR space IN (SELECT space FROM scope WHERE user = $auth.id AND scope_name = 'manage_students' GROUP BY space);
